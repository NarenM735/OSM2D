...
function onConnected() {
    ...
    stompClient.send('/ws/playerJoin',
                     {},
                     JSON.stringify({messageContent: displayName})
                    );
...
function killFeed(){
	...
	text(killFeedList[i%killFeedList.length].killer+" eliminated "+killFeedList[i%killFeedList.length].killed, width-30,80 +(i%killFeedList.length)*20);
...
function drawtimerDialog(time) {
    let x = 20, y = height-90, w = 180, h = 60;
